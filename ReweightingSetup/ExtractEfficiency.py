...
import scipy
rootfile  = 'limitForest_nominal.root'
...
def WriteEff():
    froot = TFile("original.root","RECREATE")
    
    print 'inside write eff'
    fout = open('efficiency-zpb.txt', 'w' )
    for mzp in ZpMass:
        for ma0 in A0Mass:
            print "inside loop"
            ...
            treename = 'BarZp_' + mzp_ + '_' + ma0_ +'_signal'
            ...
            #histname_up   = histname + '_DoubleBUp'
            #histname_down = histname + '_DoubleBDown'
...
def SaveHisto(model='zpb'):
    print "inside savehisto"
    fhisto = fileutils.OpenRootFile('efficiency-zpb.root','RECREATE')
    h_eff = TH2F('h_eff_'+model, 'h_eff_'+model, 20 , 600,2600, 7,300,900 )
    
    for iline in open('efficiency-zpb.txt'):
        masses = iline.rstrip().split()
