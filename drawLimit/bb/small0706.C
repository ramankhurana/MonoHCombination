...
TH2D* small0706Compare(string inputDir[],string outputName,int option=0,int retrunexp=0){
		...
		for(int j=0;j<6;j++){
				...
				for(Long64_t jEntry=jEntryMax; jEntry>-1;jEntry--){
					data.GetEntry(jEntry);
						...
						if(quantileExpected==-1){
							if(i==3 && j==1)cout<<i<<","<<j<<","<<(limit[0]<limit[1]?1:0)<<","<<limit[0]<<","<<limit[1]<<endl;
							
							isData1=(limit[0]<limit[1]||limit[0]==limit[1])?1:0;
							th2[4]->Fill(i,j,(limit[0]<limit[1]||limit[0]==limit[1])?0.1:1);
							if(isData1)th2[1]->Fill(i,j,limit[0]);
							else th2[1]->Fill(i,j,limit[1]);
   ...
   int ple[2]={3,5};
    gStyle->SetPalette(2,ple);
    //th2[3]->Draw("TEXTza");
   th2[4]->Draw("colz");
   //gPad->Update();
    //TPaletteAxis* tp=(TPaletteAxis*)th2[4]->GetListOfFunctions()->FindObject("palette");
    //tp->SetLabelColor(0);
    //tp->SetFillColor(0);
    //tp->SetLineColor(0);
    //tp->SetY2NDC(0.5);
   
   //th2[4]->GetZaxis()->Draw("same");
