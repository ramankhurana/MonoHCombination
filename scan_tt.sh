...
mzp=`echo $1 | sed -e "s/combocards\/MB_2HDM_tt\/Zprime550A300\/cmb\/300\/DataCard_2HDM_M//g"  | sed -e "s/GeV_MonoHTauTau_13TeV.txt//g" | sed 's/_/ /g' | gawk '{print $1}'`
ma0=`echo $1 | sed -e "s/combocards\/MB_2HDM_tt\/Zprime550A300\/cmb\/300\/DataCard_2HDM_M//g"  | sed -e "s/GeV_MonoHTauTau_13TeV.txt//g" | sed 's/_/ /g' | gawk '{print $2}'`
...
echo "$mzp $ma0 ${mediator} ${dm} ${twosigdown} ${onesigdown} ${exp} ${onesigup} ${twosigup} ${observed}" >> bin/limits_2hdm_tt.txt
